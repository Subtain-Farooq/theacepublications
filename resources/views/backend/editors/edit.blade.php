    ...
    <div class="w-full">
        ...
        <div class="flex -mx-3">
            <div class="w-4/5 px-3 ">
                <div class="border px-8 py-10 rounded">
                    <form class="w-full -my-3" method="POST" action="{{ route('console.editors.update', ['id' => $editor->id]) }}" enctype="multipart/form-data">
                        @csrf
            ...
            <div class="flex items-center justify-center px-3 w-1/5">
                <div class="border px-8 py-10 rounded">
                    @if($editor->image)
                        <img class="w-64 h-64" src="{{ url($editor->image->path.$editor->image->name) }}">
                    @else
                        <img class="w-64 h-64" src="{{ url('images/avatar.jpeg') }}">
                    @endif
                </div>
            </div>
        </div>
