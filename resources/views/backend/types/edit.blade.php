    ...
    <div class="bg-white p-8 rounded">
        <div class="w-full">
            <div class="flex justify-between">
                <h3 class="text-2xl mb-4">Update Category</h3>
            </div>

            <div class="w-full border px-8 py-10 rounded">
                <form class="w-full -my-3" method="POST" action="{{ route('console.types.update', ['id' => $type->id]) }}">
                    @csrf
                    <div class="flex items-center my-3">
                        ...
                        <div class="w-4/5">
                            <input type="text" name="name" class="w-full px-4 py-2 border rounded focus:outline-none @error('name') border-red-400 hover:bg-red-100  @else focus:border-black hover:border-blue-400 bg-white @enderror" value="{{ !is_null(old('name')) ? old('name') : $type->name }}">
                            @error('name')
                        ...
                        <div class="w-4/5">
                            <label class="flex justify-start items-center cursor-pointer">
                                <div class="bg-white border rounded border-gray-400 w-6 h-6 flex flex-shrink-0 justify-center items-center mr-2 focus-within:border-blue-500">
                                    @if($type->status)
                                        <input type="checkbox" name="status" class="opacity-0 absolute" value="1" checked>
                    ...
                    <div class="flex items-center mt-10 mb-2 -mx-1">
                        ...
                        <a href="{{ route('console.types')}}" class="inline-block bg-red-400 px-5 py-2 rounded capitalize mx-1 hover:bg-red-500">Cancel</a>
                    </div>
