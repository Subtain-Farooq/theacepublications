...
@section('content')
    ...
    <div class="bg-white p-6 rounded">
        <div class="w-full">
            <div class="">
                <h3 class="text-2xl mb-3">Deleted Categories</h3>
            </div>
            @if(session('alert'))
                ...
                <table class="w-full border">
                    ...
                    @foreach ($types as $type)
                        <tr>
                            ...
                            <td class="border px-4 py-2 capitalize">{{ $type->name }}</td>
                            ...
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    <form class="inline" method="POST" action="{{ route('console.types.restore', ['id' => $type->id]) }}">
                                        @csrf
                                        <button type="submit" class="text-green-500 mx-2 hover:underline">Recover</button>
                                    </form>
                                    <form class="inline" method="POST" action="{{ route('console.types.delete', ['id' => $type->id]) }}">
                                        @csrf
