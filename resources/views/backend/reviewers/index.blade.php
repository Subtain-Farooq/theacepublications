...
@section('content')
    ...
    <div class="bg-white p-6 rounded">
        <div class="w-full">
            <div class="">
                <h3 class="text-2xl mb-3">All Reviewers</h3>
            </div>
            @if(session('alert'))
                ...
                <table class="w-full border">
                    ...
                    @foreach ($reviewers as $reviewer)
                        <tr class="hover:bg-gray-100">
                            ...
                            <td class="border px-4 py-2 capitalize text-center">{{ $reviewer->name }}</td>
                            <td class="border px-4 py-2 text-center">{{ $reviewer->email }}</td>
                            <td class="border px-4 py-2 text-center">{{ $reviewer->contact_number }}</td>
                            <td class="border px-4 py-2 text-center">{{ $reviewer->country->name }}</td>
                            ...
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    <a href="{{ route('console.reviewer.show', ['id' => $reviewer->id] ) }}" class="text-blue-500 mx-2 hover:underline">View</a>
                                    <a href="{{ route('console.reviewer.edit', ['id' => $reviewer->id] ) }}" class="text-green-500 mx-2 hover:underline">Edit</a>
