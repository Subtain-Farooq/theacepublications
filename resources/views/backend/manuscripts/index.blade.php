        ...
        <div class="w-full">
            <div class="flex justify-between">
                ...
                <div>
                    ...
                    <a href="@if(request()->is('manuscripts/pending')) # @else {{ route('console.manuscripts', ['status' => 'pending']) }} @endif" class="inline-block px-3 py-1 rounded @if(request()->is('manuscripts/pending')) bg-purple-400 cursor-default @else bg-purple-200 hover:bg-purple-300 @endif">Payment Awaited</a>
                </div>
            </div>
            @if(session('alert'))
            ...
            @if($manuscripts->isNotEmpty() && !request()->is('manuscripts/pending'))
                <table class="w-full border">
                    ...
                    @foreach ($manuscripts as $manuscript)
                        <tr class="hover:bg-gray-100">
                            <td class="border px-4 py-2 w-12 text-center">{{ $loop->iteration }}</td>
                            <td class="border px-4 py-2 capitalize w-32 text-center">{{ $manuscript->code }}</td>
                            <td class="border px-4 py-2 text-left max-w-lg">{{ $manuscript->title }}</td>
            ...
            @elseif($manuscripts->isNotEmpty() && request()->is('manuscripts/pending'))
                <table class="w-full border">
                    <thead>
                    <tr class="bg-gray-200">
                        <th class="px-4 py-2">#</th>
                    ...
                    <tbody>
                    @foreach ($manuscripts as $invoice)
                        <tr class="hover:bg-gray-100">
                            <td class="border px-4 py-2 w-12 text-center">{{ $loop->iteration }}</td>
                            <td class="border px-4 py-2 capitalize w-32 text-center">{{ $invoice->manuscript->code }}</td>
                            <td class="border px-4 py-2 text-left max-w-lg">{{ $invoice->manuscript->title }}</td>
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    @if($invoice->manuscript->status == 'submitted')
                                        <span class="text-orange-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @elseif($invoice->manuscript->status == 'payment awaited')
                                        <span class="text-purple-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @elseif($invoice->manuscript->status == 'in review')
                                        <span class="text-teal-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @elseif($invoice->manuscript->status == 'review in progress')
                                        <span class="text-green-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @elseif($invoice->manuscript->status == 'accepted')
                                        <span class="text-blue-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @elseif($invoice->manuscript->status == 'rejected')
                                        <span class="text-red-500 capitalize">
                                            {{ $invoice->manuscript->status }}
                                        </span>
                                    @endif
                            ...
                            <td class="border px-4 py-2 w-56">
                                <div class="flex justify-center">
                                    {{ $invoice->manuscript->user->name }}
                                </div>
                            </td>
                            <td class="border px-4 py-2 w-40">
                                <div class="flex justify-center">
                                    {{ $invoice->manuscript->created_at->format('M d Y') }}
                                </div>
                            </td>
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    <a href="{{ route('console.manuscripts.show', ['id' => $invoice->manuscript->id] ) }}" class="text-blue-500 mx-2 hover:underline">View</a>
                                    {{-- <a href="{{ route('console.categories.edit', ['id' => $manuscript->id] ) }}" class="text-green-500 mx-2 hover:underline">Edit</a>
                                     <form class="inline" method="POST" action="{{ route('console.categories.destroy', ['id' => $manuscript->id]) }}">
                                         @csrf
                                         <button type="submit" class="text-red-600 mx-2 hover:underline">Delete</button>
