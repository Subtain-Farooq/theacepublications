                ...
                <table class="w-full border">
                    ...
                    @foreach ($categories as $category)
                        <tr>
                            ...
                            <td class="border px-4 py-2">
                                <div class="flex justify-center">
                                    <form class="inline" method="POST" action="{{ route('console.categories.restore', ['id' => $category->id]) }}">
                                        @csrf
                                        <button type="submit" class="text-green-500 mx-2 hover:underline">Recover</button>
                                    </form>
                                    <form class="inline" method="POST" action="{{ route('console.categories.delete', ['id' => $category->id]) }}">
                                        @csrf
