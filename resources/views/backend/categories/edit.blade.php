    ...
    <div class="bg-white p-8 rounded">
        <div class="w-full">
            ...
            <div class="w-full border px-8 py-10 rounded">
                <form class="w-full -my-3" method="POST" action="{{ route('console.categories.update', ['id' => $category->id]) }}">
                    @csrf
                    <div class="flex items-center my-3">
                        ...
                        <div class="w-4/5">
                            <input type="text" name="name" class="w-full px-4 py-2 border rounded focus:outline-none @error('name') border-red-400 hover:bg-red-100  @else focus:border-black hover:border-blue-400 bg-white @enderror" value="{{ !is_null(old('name')) ? old('name') : $category->name }}">
                            @error('name')
