            ...
            <div class="my-12">
                ...
                <nav class="text-blue-700 font-bold my-8" aria-label="Breadcrumb">
                    <ol class="list-none p-0 inline-flex">
                        ...
                        <li class="flex items-center">
                            <a href="{{ route('journals.single', ['id' => $article->archive->journal->id]) }}">{{ $article->archive->journal->name }}</a>
                            <svg class="fill-current w-3 h-3 mx-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M285.476 272.971L91.132 467.314c-9.373 9.373-24.569 9.373-33.941 0l-22.667-22.667c-9.357-9.357-9.375-24.522-.04-33.901L188.505 256 34.484 101.255c-9.335-9.379-9.317-24.544.04-33.901l22.667-22.667c9.373-9.373 24.569-9.373 33.941 0L285.475 239.03c9.373 9.372 9.373 24.568.001 33.941z"/></svg>
                        </li>
                        <li>
                ...
                {{--<div class="my-8">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-2xl text-gray-800 capitalize tracking-wide">{{ $article->archive->journal->name }}</h2>
                            <div class="">
                                <p class="text-gray-700 tracking-wider leading-loose">
                                    <span class="font-bold">Year:</span> {{ $article->archive->year }} |
                                    <span class="font-bold">Volume:</span> {{ $article->archive->volume }} |
                                    <span class="font-bold">Issue:</span> {{ $article->archive->issue_number }} |
                        ...
                        <div>
                            <a href="{{ url($article->document->path. $article->document->name) }}" class="border-2 border-blue-800 px-6 py-3 tracking-wider text-lg font-bold text-blue-800 hover:bg-blue-800 hover:text-white" download>Download Article</a>
                        </div>
                    </div>
                </div>--}}
                ...
                <div class="my-8">
                    <div class="flex justify-between border-2 border-blue-800 p-8">
                        ...
                        <div class="flex flex-col justify-between">
                            <div>
                                <h2 class="text-3xl text-gray-800 mb-3 tracking-wide">{{ $article->archive->journal->name }}</h2>
                                <div class="">
                                    <p class="text-gray-700 tracking-wider leading-loose">
                                        <span class="font-bold">Year:</span> {{ $article->archive->year }} |
                                        <span class="font-bold">Volume:</span> {{ $article->archive->volume }} |
                                ...
                                @endif
                                <p class="text-gray-700  tracking-wider leading-loose">
                                    <span class="font-bold">Code:</span> <a href="{{ $article->code }}" class="underline text-blue-700 hover:no-underline hover:text-blue-800">{{ $article->code }}</a>
                                </p>
                            </div>
                            <div>
                        ...
                        <div>
                            <img src="{{ url($article->archive->journal->image->path.'/'.$article->archive->journal->image->name) }}" class="border-2 w-40 object-left ">
                        </div>
                    </div>
                </div>

                <div class="my-8 pt-2">
                    <h1 class="text-3xl text-blue-800 text-justify tracking-wide leading-tight border-l-4 border-blue-800 pl-4">{{ $article->title }} <span class="text-xs text-red-600">(Archived)</span></h1>
                ...
                <div class="my-8 bg-gray-200 px-10 py-8">
                    ...
                    @endif
                    <div class="mt-2">
                        <span class="font-bold text-blue-900 tracking-wider">Code:</span> <a href="{{ $article->code }}" class="underline text-blue-700 hover:no-underline hover:text-blue-800 pl-1">{{ $article->code }}</a>
                    </div>
                    <div>
